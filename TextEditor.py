...
class SimpleEditor(Frame):
    def __init__(self, parent=None, file=None):
        ...
        self.buatNamafile()
        parent.title("Ryfan Text Editor")
        self.buatTombol()
        self.kolomTeksUtama()
        self.indeks = 1.0
        self.path =''
    ...
    def perintahSimpan(self):
        ...
        if self.path:
            ...
            messagebox.showinfo('Berhasil', 'Selamat File Berhasil Disimpan ! ')
        else:
            tipeFile = [('Text file', '*.txt'),('Python file','*asdf.py'), ('All Files', '.*'), ('Php file', '*.php'), ('Html file', '*.html')]
    ...
    def perintahKeluar(self):
        ans = askokcancel('Keluar dari aplikasi', "anda yakin ingin keluar? Pastikan File Yang Anda buat Sudah Tersimpan")
    ...
    def openFile(self):
        extensiFile = [ ('All files','*'),('Text files','*.txt'),('Python files','*.py'), ('Php file', '*.php'), ('Html file', '*.html')]
